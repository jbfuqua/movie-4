    ...
    properties: {
        ...
        art_direction: {
            ...
            properties: {
                medium: {
                    type: 'string',
                    description: 'The physical production technique of the era, stated as a medium: e.g. "hand-painted gouache one-sheet", "screen-printed offset lithograph", "airbrushed illustration on board", "photographic composite shot on film". This is the single most important field for era authenticity. It MUST be a real, physical, PRE-DIGITAL or PRACTICAL technique — paint on board, or a photograph of real practical effects on film. NEVER a 3D render, CGI, digital painting, digital composite, or concept-art / video-game key art. That render look is exactly what makes a poster read as AI-generated.'
                },
                process: {
                    type: 'string',
                ...
                palette: {
                    type: 'array',
                    description: 'Three to five colors, named in plain language (e.g. "sodium-vapor amber"), not hex. These are PRINTING INKS — a small, limited set the decade\'s press could actually strike, muted by the paper, not the saturated glow of a backlit screen. Avoid the modern digital grade: no magenta-and-acid-green split-tone, no teal-and-orange, no clean neon.',
                    items: { type: 'string' }
                },
                composition: {
                    type: 'string',
...
const PROVOCATIONS = [
    ...
    // Restrained, composed compositions — the way a real one-sheet often sells,
    // with one calm image rather than maximal chaos. These pull the studio's feed
    // away from the loud, everything-at-once look and toward the quiet authority of
    // a real period poster. The monster is still SHOWN; it is just looked at, not
    // screamed. This range is also most of what makes the "subtly silly" land — a
    // dead-calm poster with one wrong thing in it beats a garish one every time.
    'A single composed painted portrait of the monster, still and staring straight out, the way a real one-sheet centres its creature — no chaos, no motion, just the thing, looked at.',
    'Mostly negative space and one quiet colour, the monster small but unmistakable within it. The restraint is the threat.',
    'A bold, near-graphic treatment: the monster as a strong flat silhouette against a single flat colour, the way a confident one-sheet sells on one shape.',
    'An utterly ordinary period scene, painted calm and straight, with exactly one thing wrong in it that you notice a beat too late.',
    'The two-tier one-sheet: the monster looming quietly across the top, the small human world going about its business below, not yet looking up.'];
...
export const MONSTER_MECHANISMS = [
...
export const TONES = {
    ...
    unhinged: {
        ...
        artwork: [
            ...
            '- The wildness must be STRUCTURAL, never graphic. Get it from: impossible scale, biological wrongness, dream logic, a composition that should not work and does, perspective that lurches, and colour that SCREAMS — but screams IN INK: the lurid, clashing, saturated colour of a grindhouse one-sheet or a Bava gel-lit horror, struck hot in a few loud printing inks and slightly out of register. NOT the clean neon glow of a screen, and never the magenta-and-acid-green digital grade.',
...
// The decades whose posters were PAINTED or PHOTOGRAPHED rather than digitally
// composited. This is the single biggest lever on "it looks AI-generated": a
// modern-digital poster (slick 3D render, HDR glow, digital composite) lives in
// the exact same visual space as AI art and is indistinguishable from it, while a
// hand-painted or practically-photographed poster has a physical signature the
// model must fake — and faking it even passably reads as "real old poster". So
// the studio lives here by default, weighted toward the fully hand-made eras.
// (2000s+ are reachable only when the user picks them explicitly.)
const PAINTERLY_DECADES = ['1950s', '1950s', '1960s', '1960s', '1970s', '1970s', '1980s', '1980s', '1990s'];

// A real poster-making TRADITION to aim the artwork at, rotated by seed. This is
// the strongest authenticity lever available without feeding the model actual
// reference scans: "hand-painted gouache" is a vague instruction the model
// answers with its generic idea of old-poster; naming a concrete craft tradition
// — and how it composes, not just what it is made of — gives it a real target and
// spreads the studio's output across the ways posters were actually made. Kept
// era-neutral (the decade is specified separately) and weighted toward the
// restrained, composed traditions, because a poster confident enough to be calm
// is both the most authentic and the funniest backdrop for a straight-faced
// absurd premise. Deliberately names no living artist — traditions, not styles to
// mimic.
const POSTER_TRADITIONS = [
    'a hand-painted one-sheet — a full-colour illustration by a working commercial poster painter, composed and sober, selling the whole film on one strong, calm image',
    'a spare, nearly typographic one-sheet — mostly negative space, a small but exact image, the restraint of a poster confident enough to whisper',
    'a bold graphic treatment — strong flat shapes and a tight, limited palette, the creature reduced to a near-silhouette, design doing the selling',
    'a photographic one-sheet — one strong, practical, plainly-lit photograph on film, with the type set cleanly over it',
    'a painted, slightly lurid drive-in one-sheet — bold and a little garish, struck in a few loud inks, but PAINTED by hand and never rendered'
];
...
export function conceptPrompt({ genre, era, plot, tone, avoidTitles = [], seed, idea = null }) {
    const toneRules = toneSpec(tone);
    ...
    // A chosen idea — picked by the user from a brainstormed slate — is the
    // strongest input there is: it FIXES the title and the monster (the two things
    // it was picked for) and hands us a logline to develop. This is the wire that
    // connects the idea-picker to the poster; without it the two flows invent
    // unrelated films. It behaves like a locked premise, only richer.
    //
    // `locked` means "a fixed premise is driving this film" — true for a chosen
    // idea OR a typed-in premise — and it flips the diversification and
    // anti-repetition machinery below from "invent something new" to "keep this
    // film, vary only the execution".
    const chosen = idea && (idea.title || idea.monster || idea.logline) ? idea : null;
    const locked = !!chosen || !!plot;

    // On "any", pick a hand-made-era decade rather than letting the model default
    // to a modern-digital one (left alone it converges on the 2010s, which is the
    // worst case for looking AI). An explicit decade from the user always wins.
    const decade = era === 'any' ? PAINTERLY_DECADES[(seed >> 8) % PAINTERLY_DECADES.length] : era;

    // The poster-craft tradition to aim at, offset from the decade/mechanism/
    // provocation seeds so it varies independently of them.
    const tradition = POSTER_TRADITIONS[(seed >> 12) % POSTER_TRADITIONS.length];
    ...
    if (chosen) {
        lines.push(
            '',
            'THE FILM IS ALREADY CHOSEN. This exact idea was pitched against a whole slate and picked to go into production. Your job is NOT to invent a film — it is to DEVELOP this one into a finished concept and its one-sheet. Do not replace it, do not reach for a cleverer idea, and do not swap the monster out for a different one.',
            `  TITLE — keep it, this is the film's title: "${chosen.title}"`,
            `  THE MONSTER — build exactly this creature, no substitutions: ${chosen.monster}`,
            `  THE HOOK — this is the story: ${chosen.logline}`,
            '',
            'Develop it into a real film: give it a specific victim and situation, its decade, its art direction, its cast and billing, and a tagline that sells it. But the title stays, the monster stays, and what you deliver is recognisably the film that was greenlit.'
        );
    } else if (plot) {
        lines.push(
            '',
    ...
    lines.push(
        ...
        chosen
            ? 'The title and the monster are fixed. What must be fresh is everything else: the angle into the story, the victim, the single image on the poster. The first execution that occurs to you is the one that occurs to everyone — reach past it.'
            : plot
    ...
    lines.push(
        ...
        '',
        chosen
            ? `THE MONSTER IS SET: ${chosen.monster}. Build exactly that creature — give it a face, an anatomy and a way of moving — and put it on the poster, big and specific. Do not trade it for a different kind of threat. It is embodied, and it is shown.`
    ...
    lines.push(`- The film is from the ${decade}, and its poster was made with that decade's real, physical craft.`);
    ...
    if (avoidTitles.length) {
        lines.push(
            '',
            // With a premise (or a chosen idea) supplied, the user is DELIBERATELY
            // committing to this one film — so "avoid the same premise" is precisely
            // wrong here. Only the execution must differ.
            locked
                ? 'ALREADY MADE — this studio has released these. You are deliberately committing to the film above, so a similar premise is expected and correct. What must NOT repeat is the execution: do not reuse these naming patterns or these central images. Same film, told well.'
    ...
    // A chosen idea fixes the title — but copyRules is about to spend a lot of
    // words on how to WRITE a title. Head that off: the craft below is for
    // understanding why a title earns its place, and for the tagline, which the
    // pitch never wrote. The title itself stays.
    if (chosen) {
        lines.push(
            '',
            `THE TITLE IS DECIDED: "${chosen.title}". The title guidance just below is for understanding why a title works — it is NOT a licence to write a new one. Keep this title exactly. Put that craft into a fresh TAGLINE that sells it.`
        );
    }
    ...
    lines.push(
        ...
        '',
        // The single most important instruction for not looking AI-generated. The
        // image model's default — a slick 3D render / digital composite — is the
        // exact thing that reads as AI, because AI art and modern digital art are
        // the same visual space. Force the poster into a PHYSICAL medium instead.
        'PAINTED OR PHOTOGRAPHED — NEVER RENDERED. This is what separates a real poster from an AI-generated one, so it is not optional. The artwork is made in ONE of exactly two ways, whichever fits the decade:',
        '  • PAINTED — a hand-made illustration in a real medium (gouache, acrylic, oil, airbrushed paint on illustration board), with an illustrator\'s visible hand and brushwork; or',
        '  • PHOTOGRAPHED — a real photograph shot on film, of real actors and PRACTICAL effects: a built creature suit, an animatronic, a model or miniature, a matte painting, lit in-camera.',
        'So the monster is EITHER painted by hand OR a physical thing that was built and photographed on film. It is NEVER a 3D computer render, a CGI creature, a digital sculpt with smooth waxy subsurface-scatter skin, a "digital painting", ArtStation or concept-art or video-game key art, or a glossy digital composite with HDR glow and lens flares. Those ARE the AI look this studio is escaping. Write art_direction.medium and .process to name a specific, real, physical (painted or practical-photographic) technique, and commit to it.',
        '',
        `THE TRADITION to aim this poster at — build the art_direction around it, realised in ${decade} craft: ${tradition}`,
        '',
        // The register fix and the "subtly silly" engine in one. Restraint is the
        // default (a tone can turn the volume up); playing it dead straight is
        // universal. This is what makes it read as a real movie AND what makes the
        // absurd premise funny — the deadpan gap only survives if the poster never
        // winks. Named last so it is the final word on execution.
        'PLAY IT COMPLETELY STRAIGHT — and, by default, QUIET. This is the whole game. The poster is a real film\'s real advertising and it believes itself utterly: sober, composed, made with real craft, exactly like an actual period one-sheet. Unless the tone above has explicitly told you to turn the volume up, the default is RESTRAINT — most real posters sell with ONE strong, calm image and confident type, not with everything cranked at once, and that quiet authority is what makes a stranger believe it is a real movie. Whatever the volume, it NEVER winks: if the premise is faintly absurd, the artwork and the copy present it with total sincerity, as straight as a serious film would, and never let on. That deadpan gap — dead-straight execution, a quietly ridiculous premise — is the entire joke, and it only survives if the poster itself never gets in on it.'
    );
...
export function imagePrompt(concept, { creditsMode = 'model' } = {}) {
    ...
    return [
        ...
        `A photograph of an original ${concept.decade} ${concept.genre} movie poster — a real one-sheet that was printed on paper at the time and has survived to be photographed today. It is a physical object, not a digital picture of one. Portrait orientation.`,
        '',
        // gpt-image-2's default is to OVER-RENDER: every surface equally sharp,
        // equally lit, glossy, hyperreal. That uniform perfection is the entire
        // "AI look". The strongest single counter is to state, positively and
        // early, that a real analog process yields LIMITED, UNEVEN detail. A
        // negative list alone (below) is weighted too weakly to beat the prior.
        'READ THIS FIRST — IT OVERRIDES THE DEFAULT LOOK. This poster was made by a real, imperfect, analog process and it must look it. Real printed art has LIMITED and UNEVEN detail: broad passages of flat, unmodulated colour; detail that simplifies and is lost in the shadows; edges that are not razor-clean; one consistent direction of light. This is the OPPOSITE of a digital render where every surface is equally crisp, equally lit and equally detailed. Aim for the real thing: fewer details, but each one made of paint or ink or grain.',
        '',
        // The single strongest lever against the AI look: forbid the render idiom
        // and force a physical medium. The concept layer already commits to one in
        // art_direction.medium; this restates it as a hard rule for the renderer,
        // which otherwise defaults straight back to a 3D/CGI composite.
        'PAINTED OR PHOTOGRAPHED — NEVER RENDERED. This poster\'s art was made by hand or by camera, the way posters were made before computers did it. If it is illustration, it is real PAINT on board — visible brushwork, the flatness of pigment, an illustrator\'s hand. If it is photographic, it is a real photograph on film of real actors and PRACTICAL effects — a built creature, a suit, an animatronic, a miniature, a matte painting. It is NOT a 3D computer render, NOT a CGI creature, NOT a digital sculpt with smooth waxy subsurface-scatter skin, NOT a "digital painting", NOT ArtStation concept art or video-game key art, and NOT a glossy digital composite with HDR glow or lens flares. Paint or photograph — nothing in between.',
        '',
        'THE ARTWORK ON THE POSTER',
        `Medium: ${art.medium}`,
        `Subject: ${art.subject}`,
        `Environment: ${art.environment}`,
        `Lighting: ${art.lighting}. One dominant light source; everything else falls into genuine shadow. No soft, even, all-over illumination.`,
        `Palette: ${art.palette.join(', ')}. A period print was struck from a LIMITED set of inks laid down in flat areas — not a smooth digital blend of unlimited colour. Keep the palette tight and the colour slightly muted, the way ink on paper is, never the saturated glow of a screen.`,
        `Composition: ${art.composition}`,
        '',
        'HOW IT WAS PHYSICALLY MADE — this is not decoration, it is the whole point. Every mark of the real tool and the real press MUST survive into the final image:',
        `The original art: ${art.process}. The marks of the tool are visible — the drag and ridge of the brush, the tooth of the board, the grain of the film — and some passages are left simplified and flat. It is not smoothed to an even finish.`,
        `The printing: ${art.reproduction}. It carries a visible halftone dot, colour plates that do not register perfectly, and ink that has spread very slightly into paper that is not pure white.`,
        `The object we are looking at: ${art.artifact}. This is a PHOTOGRAPH of that physical sheet, so it also carries the paper's own texture and low sheen under the light, the faint grain and softness of a real camera and lens, and the fact of being a real object in real light — not a flat, clean digital scan.`,
        '',
        ...
        creditsInstruction,
        '',
        // Negatives still earn their place as reinforcement — but each AI tell is
        // named concretely and paired with the physical thing that replaces it,
        // which lands harder than a bare "don't be digital".
        'AVOID THE AI-GENERATED LOOK. None of these belong on a real printed poster, and each must be replaced by its physical counterpart: a 3D render, CGI model or digital sculpt → a creature that was painted, or built and photographed on film; smooth waxy subsurface-scatter skin → real skin on film, or real paint; ArtStation / concept-art / video-game key-art slickness → the hand of a working commercial illustrator; airbrushed digital smoothness → visible brush, ink and grain; CGI sheen and glossy plastic surfaces → matte paint and paper; HDR glow and lens flares → the flat, limited light of real film; hyperreal detail rendered evenly across the whole frame → detail that concentrates at the focus and drops away elsewhere; flat even render-lighting with no source → a single hard light and real shadow; flawless symmetry, clean vector edges, and any mirrored or kaleidoscopic repetition → the small irregularities of a thing made by hand; smooth infinite gradients → banded, slightly mottled ink; that faint three-dimensional computer-generated quality → the unmistakable flatness of ink pressed onto paper.',
        '',
