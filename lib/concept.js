...
// Per-decade painterly style, ported from the OLD app (main branch) — the version
// whose posters actually looked like real one-sheets. Its winning move was simple,
// POSITIVE, per-decade language ("painted montage, acrylics, brushwork") rather
// than the rebuild's wall of anti-AI negatives. Positive-and-specific beats a pile
// of "don't". Leans painted for every decade, because paint is what dodges the AI
// render look; the modern decades stay photographic-or-painted, never CGI.
const STYLE_BY_DECADE = {
    '1950s': 'a hand-painted gouache one-sheet: visible brush strokes, warm slightly-muted colour, the honest imperfections of paint on illustration board',
    '1960s': 'a silkscreen graphic poster: bold flat colours, simplified shapes, offset-litho printing, strong mid-century graphic design',
    '1970s': 'an airbrushed painted illustration: soft gradients, earthy colour, hand-painted forms with visible paint texture',
    '1980s': 'a painted poster montage in acrylics: dramatic painted lighting, rich colour, confident brushwork, characters and scene painted by hand — the classic illustrated one-sheet',
    '1990s': 'a painted key-image illustration with a photographic-composite feel: illustrated and hand-finished, not digital',
    '2000s': 'a photographic key-art one-sheet or a painted illustration: real photography or real paint, never a 3D render',
    '2010s': 'a photographic key-art one-sheet: one strong real photograph, plainly lit, never a 3D render or CGI',
    '2020s': 'a photographic key-art one-sheet: one strong real photograph, plainly lit, never a 3D render or CGI'
};
...
// The bar for the title and tagline is now the CANON — real films and their real
// taglines (see canon.js) — not a set of invented exemplars.
//
// Hard lesson (2026-07-20): the invented exemplars this used to carry ("FALLOUT",
// "FREE REFILLS", "SOMETHING GOT IN") were STEERING the writer toward gimmicky,
// flat, sentence-shaped titles ("YOUR CHILD CAME HOME WRONG") — while the old app,
// which gave the writer almost no title guidance at all, produced far better
// genre titles ("Necromorph", "The Creeping Mass", "Void Titan"). Few-shot is the
// strongest signal in a brief, so bad exemplars actively poison the output. The
// fix: point at the real canon and otherwise trust the writer's instinct for what
// a real film is called.
...
export function copyRules(tone) {
    ...
    const lines = [
        '',
        'THE TITLE AND THE TAGLINE. Write them the way a real film of the era did. The bar is the canon — real films and their real taglines:',
        canonBlock(),
        '',
        '- THE TITLE: a strong, original, era-appropriate genre title — the kind a real monster or sci-fi picture of that decade actually carried. Look at the range in the canon: a loaded noun (Alien, Poltergeist), a "The ___" (The Thing, The Fly, The Blob), a name, a short phrase. Pick whatever genuinely fits THIS film, make it evocative and sayable, and trust your instinct for what a real one is called. Do not force a gimmick, and do not overthink it.',
        '- THE TAGLINE: a real hook with teeth, in that decade\'s advertising voice, like the ones above. It threatens, dares, warns or promises the reader something; it never just narrates the plot. A full stop reads stronger than an em-dash.',
        ''
    ];
...
export function imagePrompt(concept, { creditsMode = 'model' } = {}) {
    ...
    const styleHint = STYLE_BY_DECADE[concept.decade] || STYLE_BY_DECADE['1980s'];

    return [
        // Lead POSITIVELY with the medium, the way the old app did — that is what
        // produced posters that looked real. Ask for the ARTWORK of a period
        // one-sheet in a named hand-made medium, not "a photograph of a physical
        // object" (which invites the slick photoreal render we are trying to avoid).
        `The artwork of an authentic ${concept.decade} ${concept.genre} movie one-sheet — ${styleHint}. Portrait orientation.`,
        '',
        // One short, positive line instead of the old wall of anti-AI negatives.
        // Positive-and-specific beats a pile of "don't"; the old app had none of the
        // negatives and looked far better.
        'This is real, hand-made poster art — painted or photographed by people, carrying the brushwork, texture and honest imperfection of the real medium. It is illustration or photography, not a 3D render, not CGI, not a slick digital painting.',
        '',
        'THE ARTWORK',
        `Subject: ${art.subject}`,
        `Environment: ${art.environment}`,
        `Lighting: ${art.lighting}`,
        `Palette: ${art.palette.join(', ')} — a limited set of real inks, slightly muted the way print is, not the saturated glow of a screen.`,
        `Composition: ${art.composition}`,
        `Medium and handling: ${art.medium}. ${art.process}`,
        '',
        'TEXT — render these exactly as written, correctly spelled:',
        `1. The title, "${concept.title.toUpperCase()}", set in the ${tt.placement}. Lettering: ${tt.lettering}. Color: ${tt.color}. Effect: ${tt.effect}. The title is part of the artwork — lettered in the same hand and process, not pasted on top.`,
