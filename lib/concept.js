...
// The premise bar. This is the gap the last five rounds never touched: all the
// machinery judged the TITLE and TAGLINE, and nothing pushed on the film idea
// itself — so a generic idea sailed through wearing good copy. The failure mode
// is specific and it is the model's default: it reaches for ominous ABSTRACTION
// ("a heat source that has already chosen its way into the house") to sound
// frightening, instead of committing to one concrete, specific scare. Shown, not
// described: two loglines at the bar, and the rule that a real idea survives
// being said plainly.
const PREMISE_EXEMPLARS = [
    'A night-shift toll operator on an empty turnpike notices the same car pass her booth eleven times before dawn, and the driver is a little more decomposed on each pass.',
    'After the strike shutters the mine, the town\'s taps run warm, and one by one the children start describing the man who lives at the bottom of the well.'
];
...
const EXEMPLARS = [
    ...
    {
        title: 'DON\'T ANSWER IT',
        tagline: 'It hasn\'t stopped ringing since the funeral.',
        why: 'the title is a whole imperative sentence, and the tagline is the reason you should obey it.'
    },
    {
        title: 'FREE REFILLS',
...
export function conceptPrompt({ genre, era, plot, tone, avoidTitles = [], seed }) {
    ...
    lines.push(
        '',
        'THE IDEA. Before the poster, the film. The kind of premise that works:',
        ...PREMISE_EXEMPLARS.map((p) => `  • ${p}`),
        'Notice what those do: ONE specific person, in ONE specific situation, with ONE concrete thing to be afraid of — and they commit to what the thing IS or does. You could say either one out loud to a stranger and watch them lean in.',
        'The failure to avoid is the opposite reflex, and it is the one you will reach for first: fogging the scare over to make it sound deep. "A presence that has chosen its way into the house." "Something that should not be." That vagueness is the sound of an idea that is not there yet. If you cannot name plainly what the threat is and what it does, you have not finished inventing it. A real premise frightens even when described flatly.'
    );
