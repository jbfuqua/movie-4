...
export function checkTagline(tagline) {
    ...
    // "It rose in the night. It has been rising ever since." — two drifting
    // sentences with no hook is the signature of the mood-piece failure.
...
export function checkCopy(concept) {
    return [checkTitle(concept?.title), checkTagline(concept?.tagline)].filter(Boolean);
}
